...
All default scenarios are in the src/games directory. New scenarios can be written by creating a new .json file. The file should contain a single JSON object representing the project as a whole. The format for the file is as follows:
```json
{
...
Once a new scenario has been created, it should be available in game after a rebuild has been performed. Alternatively a
user could manually move their .json scenario file into bin/games/.
